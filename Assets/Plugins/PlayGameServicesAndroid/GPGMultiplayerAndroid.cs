...
namespace Prime31
{
	public class GPGMultiplayer
	{
		...
		// Gets all the participants in the current room. Note that the includeCurrentPlayer filter is currently Android only.
		public static List<GPGMultiplayerParticipant> getParticipants( bool includeCurrentPlayer = false )
		{
			if( Application.platform != RuntimePlatform.Android )
				return null;

			var json = _plugin.Call<string>( "getParticipants", includeCurrentPlayer );
